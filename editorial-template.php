...
<body>
<?php $pageType = 'editorial'; require __DIR__ . '/header.php'; ?>
<main class="editorial-page<?= !empty($page['compact']) ? ' editorial-page--compact' : '' ?>">
  <section class="editorial-hero">
    ...
    <div class="editorial-hero-copy"><span><?= htmlspecialchars($page['eyebrow']) ?></span><h1><?= $page['title'] ?></h1><p><?= htmlspecialchars($page['description']) ?></p></div>
  ...
  <section class="faq-list" aria-label="Frequently asked questions">
    <?php foreach ($page['faqs'] as $index => $faq): ?><details class="pdp-reveal"<?= $index === 0 ? ' open' : '' ?>><summary><?= htmlspecialchars($faq['q']) ?><span aria-hidden="true">+</span></summary><p><?= htmlspecialchars($faq['a']) ?></p></details><?php endforeach; ?>
  </section>
  <?php endif; ?>

  <?php if (!empty($page['note'])): ?><aside class="editorial-note pdp-reveal"><span>Important</span><p><?= htmlspecialchars($page['note']) ?></p></aside><?php endif; ?>
